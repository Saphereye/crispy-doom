           ...
           boolean dropped) // [NS] Dropped ammo/weapons give half as much.
{
    ...
    if (gameskill == sk_baby || gameskill == sk_nightmare || critical->moreammo)
    {
...
const char *const WeaponPickupMessages[NUMWEAPONS] = {
    NULL, // wp_fist
    NULL, // wp_pistol
    GOTSHOTGUN, GOTCHAINGUN, GOTLAUNCHER, GOTPLASMA,
    GOTBFG9000, GOTCHAINSAW, GOTSHOTGUN2,
};
...
void P_GiveCard(player_t *player, card_t card)
{
    ...
    player->bonuscount +=
        netgame ? BONUSADD : 0; // [crispy] Fix "Key pickup resets palette"
    player->cards[card] = 1;
}
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
	...
	*/
    ...
    player->bonuscount += BONUSADD;
    if (player == &players[displayplayer])
        S_StartSoundOptional(NULL, sound,
                             sfx_itemup); // [NS] Fallback to itemup.
}
...
void P_KillMobj(mobj_t *source, mobj_t *target)
{
    ...
    if (target->player)
    {
        ...
        target->player->fixedcolormap =
            target->player->powers[pw_infrared] ? 1 : 0;
